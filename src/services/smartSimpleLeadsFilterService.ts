...
import { SmartSimpleLeadsFilters } from '../hooks/useSmartSimpleLeadsFilters';

export interface SmartSimpleFilteredLeadsResult {
  leads: any[];
  totalCount: number;
  kpis: {
    total: number;
...
export const fetchSmartSimpleFilteredLeads = async (
  userId: string,
  filters: SmartSimpleLeadsFilters,
  page: number = 1,
  pageSize: number = 25
): Promise<SmartSimpleFilteredLeadsResult> => {
  try {
    console.log('[SmartSimpleLeadsFilterService] Fetching filtered leads with:', { filters, page, pageSize });
    ...
    if (error) {
      console.error('[SmartSimpleLeadsFilterService] Error fetching leads:', error);
      throw error;
    }
    ...
    console.error('[SmartSimpleLeadsFilterService] Error:', error);
    throw error;
  }
};
