-- EMERGENCY FIX: Complete permission bypass and trigger fix
...
-- 3) Create a super robust trigger function that bypasses all security
...
BEGIN
  ...
  -- Force insert the user record with explicit error handling
  BEGIN
    ...
    RAISE NOTICE '✅ SUCCESS: User record created for % with ID %', NEW.email, NEW.id;
  ...
  EXCEPTION WHEN unique_violation THEN
    ...
    RAISE NOTICE '✅ UPDATED: User record updated for % with ID %', NEW.email, NEW.id;
    
  EXCEPTION WHEN OTHERS THEN
    -- Log the error but don't fail
    RAISE WARNING '⚠️ Error creating user record for %: %', NEW.email, SQLERRM;
  END;
