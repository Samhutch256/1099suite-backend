-- Fixed mileage tracking tables migration
-- This migration works with the existing SQLite schema and creates Supabase tables

-- First, check if mileage_trips table exists and drop it if it does
DROP TABLE IF EXISTS mileage_trip_points CASCADE;
DROP TABLE IF EXISTS mileage_trips CASCADE;

-- Create mileage_trips table with UUID (Supabase standard)
...
CREATE INDEX idx_mileage_trips_started_at ON mileage_trips(started_at DESC);
CREATE INDEX idx_mileage_trips_classification ON mileage_trips(classification);
CREATE INDEX idx_mileage_trip_points_trip_id ON mileage_trip_points(trip_id);
CREATE INDEX idx_mileage_trip_points_timestamp ON mileage_trip_points(timestamp);
